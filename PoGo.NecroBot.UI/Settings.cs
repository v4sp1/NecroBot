...
namespace PoGo.NecroBot.UI
{
    internal class AuthSettings
    {
        ...
        public void Load(string path)
        {
            ...
            {
                switch (MessageBox.Show("Which authentication type?\nYes\tGoogle\nNo\tPokémon Trainer Club\nCancel\tAbort", "NecroBot", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question))
                {
                    ...
                    case DialogResult.No:
                        ...
                        do
                        {
                            PtcUsername = Microsoft.VisualBasic.Interaction.InputBox("Enter your PTC username.", "NecroBot");
                            if (PtcUsername == "")
                                Environment.Exit(2);
                        }
                        while (string.IsNullOrWhiteSpace(PtcUsername));
                        do
                        {
                            PtcPassword = Microsoft.VisualBasic.Interaction.InputBox("Enter your PTC password.", "NecroBot");
                            if (PtcPassword == "")
                                Environment.Exit(2);
                        }
                        while (string.IsNullOrWhiteSpace(PtcPassword));
                        break;
                    default:
                        Environment.Exit(2);
                        break;
